    ...
    try {
        ...
        if (endpoint.includes('/auth/login')) {
            ...
            if (endpoint === '/conversation/start') {
                return {
                    message: `Ciao ${JSON.parse(options.body).patientName.split(' ')[0]}, sono EmoBial e vorrei farti alcune brevi domande per capire meglio come ti senti. Le tue domande saranno riservate e serviranno solo per aiutare il medico a capire meglio il tuo stato emotivo. Possiamo iniziare?`,
                    conversationState: {
                        currentScale: 'GAD-7',
