...
function getGADInterpretation(score) {
    if (isNaN(score)) {
        console.error("GAD: Punteggio non valido");
    ...
    if (score >= 15 && score <= 21) return 'Ansia severa';
    
    console.error(`GAD: Punteggio fuori intervallo ${score}`);
    return 'Punteggio non classificabile';
}


function getPHQInterpretation(score) {
    if (isNaN(score)) {
        console.error("PHQ: Punteggio non valido");
        return "Punteggio non valido";
    }
    ...
    if (score >= 20 && score <= 27) return 'Depressione severa';
    
    console.error(`PHQ: Punteggio fuori intervallo ${score}`);
    return 'Punteggio non classificabile';
}

// Funzione per assegnare classi di severità
function getSeverityClass(score, type) {
    if (type === 'GAD') {
        if (score >= 0 && score <= 4) return "badge bg-success";  // Verde per ansia minima
        if (score >= 5 && score <= 9) return "badge bg-primary";  // Blu per ansia lieve
        if (score >= 10 && score <= 14) return "badge bg-warning"; // Giallo per ansia moderata
        return "badge bg-danger";  // Rosso per ansia severa
    }
    if (type === 'PHQ') {
        if (score >= 0 && score <= 4) return "badge bg-success";  // Verde per depressione minima
        if (score >= 5 && score <= 9) return "badge bg-primary";  // Blu per depressione lieve
        if (score >= 10 && score <= 14) return "badge bg-warning"; // Giallo per depressione moderata
        if (score >= 15 && score <= 19) return "badge bg-warning"; // Arancione per depressione moderata-severa
        return "badge bg-danger";  // Rosso per depressione severa
    }
}
...
function viewAssessmentResults(assessment) {
    console.log("DEBUG: Oggetto assessment completo:", assessment);
    
    // Verifica dettagliata delle proprietà dell'assessment
    console.log("DEBUG: Proprietà specifiche:", {
        gadScore: assessment.gadScore,
        phqScore: assessment.phqScore,
        date: assessment.date
    });

    // Controllo di validità
    if (!assessment || typeof assessment.gadScore === 'undefined' || typeof assessment.phqScore === 'undefined') {
        console.error("Errore: Dati assessment non validi");
        showErrorNotification("Impossibile visualizzare i risultati");
        return;
    }
...
function updateResultsUI(assessment) {
    console.log("DEBUG: Aggiornamento UI con i risultati:", assessment);
    
    // Ottieni gli elementi UI con log di debug
    const resultElements = {
        patientName: document.getElementById('resultPatientName'),
        assessmentDate: document.getElementById('assessmentDate'),
        gadScoreValue: document.getElementById('gadScoreValue'),
        phqScoreValue: document.getElementById('phqScoreValue'),
        gadInterpretation: document.getElementById('gadInterpretation'),
        phqInterpretation: document.getElementById('phqInterpretation')
    };

    // Log degli elementi trovati
    Object.entries(resultElements).forEach(([key, element]) => {
        console.log(`DEBUG: Elemento ${key}:`, element ? "TROVATO" : "NON TROVATO");
    });
    
    // Aggiorna il nome del paziente se disponibile
    if (resultElements.patientName && appState.currentPatient) {
        resultElements.patientName.textContent = `${appState.currentPatient.lastName} ${appState.currentPatient.firstName}`;
    }
    
    // Aggiorna la data della valutazione
    if (resultElements.assessmentDate) {
        resultElements.assessmentDate.textContent = new Date(assessment.date).toLocaleDateString('it-IT');
    }
    
    // Verifica e aggiornamento dei punteggi
    if (resultElements.gadScoreValue && resultElements.phqScoreValue) {
        // Conversione esplicita e log dei valori
        const gadScore = Number(assessment.gadScore);
        const phqScore = Number(assessment.phqScore);
        
        console.log("DEBUG: Punteggi convertiti:", {
            gadScore,
            phqScore
        });

        resultElements.gadScoreValue.textContent = gadScore;
        resultElements.phqScoreValue.textContent = phqScore;
        
        // Calcola e imposta l'interpretazione
        const gadInterpretationText = getGADInterpretation(gadScore);
        const phqInterpretationText = getPHQInterpretation(phqScore);
        
        console.log("DEBUG: Interpretazioni:", {
            gadInterpretationText,
            phqInterpretationText
        });

        if (resultElements.gadInterpretation) {
            resultElements.gadInterpretation.textContent = gadInterpretationText;
        }
        
        if (resultElements.phqInterpretation) {
            resultElements.phqInterpretation.textContent = phqInterpretationText;
        }
    } else {
        console.error("Errore: Elementi per i punteggi non trovati");
        showErrorNotification("Impossibile mostrare i risultati dettagliati");
    }
}
